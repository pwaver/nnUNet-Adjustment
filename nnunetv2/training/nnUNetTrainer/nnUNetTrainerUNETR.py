...
class nnUNetTrainerUNETR(nnUNetTrainerNoDeepSupervision):
    ...
    def build_network_architecture(
        ...
        # configuration_manager = img_size = configuration_manager.patch_size
        img_size=[512,512]
        spatial_dims = len(img_size)

        print(init_kwargs)
        print(init_kwargs_req_import)
        print("patch_size in build_network_architecture", img_size)
        # self.configuration_manager.patch_size = patch_size
        model = UNETR(
            in_channels = num_input_channels,
            out_channels = num_segmentation_heads,
            img_size = img_size,
            feature_size=16,
            hidden_size=768,
            mlp_dim = 3072,
